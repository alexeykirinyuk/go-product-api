...
//
// Each mapping specifies a URL path Product and an HTTP method. The path
// Product may refer to one or more fields in the gRPC request message, as long
// as each field is a non-repeated field with a primitive (non-message) type.
// The path Product controls how fields of the request message are mapped to
// the URL path.
//
// Example:
//
...
//
// Any fields in the request message which are not bound by the path Product
...
// every field not bound by the path Product should be mapped to the
...
//     - Fields referred by the path Product. They are passed via the URL path.
...
// Product. A variable Product must not contain other variables. If a variable
// matches a single path segment, its Product may be omitted, e.g. `{var}`
